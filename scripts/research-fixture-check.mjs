...
const tradeCandidatesCsv = writeFile("trade-candidates.csv", [
  "candidate_id,trade_id,entry_label_id,exit_label_id,action,target_exit,target_hold,ticker,timeframe,timestamp,bar_index,in_trade_bar_index,chart_price,entry_timestamp,exit_timestamp,entry_price,exit_price,return_pct,feature_close,feature_volume,feature_ema25,feature_sma100,feature_atr14,feature_stoch_rsi_k,feature_stoch_rsi_d,feature_close_above_ema25,feature_close_above_sma100,feature_distance_to_ema25_pct,feature_distance_to_sma100_pct,feature_recent_5_return_pct,feature_recent_10_return_pct,feature_recent_20_return_pct,feature_recent_20_high,feature_recent_20_low,feature_close_rank_recent_20,feature_paired_ticker,feature_paired_close,feature_pair_ratio_close,feature_d1_close,feature_d1_close_above_ema25,feature_h4_close,feature_h4_close_above_ema25,feature_h2_close,feature_h2_close_above_ema25",
...
try {
  ...
  assert(readFile("dataset-report.md").includes("Same-Candle Decision Conflicts"), "dataset report should include same-candle decision diagnostics");
  ...
  const inTradeSkipLabelsCsv = writeFile("in-trade-skip-labels.csv", [
    "id,label_source,training_eligible,action,ticker,timeframe,timestamp,bar_index,chart_price,execution_price,trade_id,parent_entry_label_id,capture_mode,visible_until_timestamp,potential_visual_leakage,confidence,setup_quality,reason_codes,notes,created_at",
    "bad_entry,retrospective_replay,1,ENTRY,SOXL,4H,2024-02-01T14:30:00.000Z,0,10,,bad_trade,,replay,2024-02-01T14:30:00.000Z,0,,,,2024-02-01T14:30:00.000Z",
    "bad_skip,retrospective_replay,1,SKIP,SOXL,4H,2024-02-01T18:30:00.000Z,1,10.5,,,,replay,2024-02-01T18:30:00.000Z,0,,,,2024-02-01T18:30:00.000Z",
    "bad_exit,retrospective_replay,1,EXIT,SOXL,4H,2024-02-02T14:30:00.000Z,2,11,,bad_trade,bad_entry,replay,2024-02-02T14:30:00.000Z,0,,,,2024-02-02T14:30:00.000Z"
  ].join("\n") + "\n");
  const inTradeSkipTrainingCsv = writeFile("in-trade-skip-training.csv", [
    ...
    "bad_entry,retrospective_replay,replay,ENTRY,1,0,0,0,SOXL,4H,2024-02-01T14:30:00.000Z,bad_trade,,10,,10,2024-02-01T14:30:00.000Z",
    "bad_skip,retrospective_replay,replay,SKIP,0,0,1,0,SOXL,4H,2024-02-01T18:30:00.000Z,,,10.5,,10.5,2024-02-01T18:30:00.000Z",
    "bad_exit,retrospective_replay,replay,EXIT,0,1,0,0,SOXL,4H,2024-02-02T14:30:00.000Z,bad_trade,bad_entry,11,,11,2024-02-02T14:30:00.000Z"
  ].join("\n") + "\n");
  const inTradeSkipTradesCsv = writeFile("in-trade-skip-trades.csv", [
    "trade_id,ticker,status,entry_label_id,exit_label_id,entry_timestamp,exit_timestamp,entry_price,exit_price,return_pct",
    "bad_trade,SOXL,closed,bad_entry,bad_exit,2024-02-01T14:30:00.000Z,2024-02-02T14:30:00.000Z,10,11,10"
  ].join("\n") + "\n");
  run([
    "research/dataset_report.py",
    "--labels", inTradeSkipLabelsCsv,
    "--training", inTradeSkipTrainingCsv,
    "--trades", inTradeSkipTradesCsv,
    "--output", path.join(tempDir, "in-trade-skip-report.md"),
    "--json-output", path.join(tempDir, "in-trade-skip-report.json")
  ]);
  const inTradeSkipReport = JSON.parse(readFile("in-trade-skip-report.json"));
