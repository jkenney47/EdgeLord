...
`export:backup` writes `labels.csv`, `trades.csv`, `training-features.csv`, `trade-candidates.csv`, `labels.jsonl`, `schema.json`, the API export manifest, and a backup manifest into `exports/YYYYMMDDTHHMMSSZ/`. The backup manifest embeds the parsed API export manifest and records byte counts plus SHA-256 hashes for every exported file, so a saved export folder keeps its label counts, training policy, trade status, trade-candidate coverage, and file fingerprints without rerunning the API. Export payloads are ignored by git; only `exports/.gitkeep` is tracked.
...
`data:status` runs coverage, label integrity, export backup, and the dataset readiness report as one post-import health check. Run it after importing real adjusted data and periodically while labeling. It writes `reports/<timestamp>-data-status.json` with command results, pointers to the latest coverage, integrity, and research summary artifacts, the current `nextLabelingTarget`, and a machine-readable `warnings` array for operator issues such as stale live API focus copy.

`validate:csv` checks a local adjusted OHLCV CSV for required columns, SOXL/SOXS rows, duplicate ticker/timestamps, valid dates, positive OHLCV, internally consistent OHLC values, and paired SOXL/SOXS timestamp overlap. Duplicate ticker/timestamps are import errors because the API rejects them too. If an optional `adjusted` column is present, every row must be truthy (`1`, `true`, `yes`, `y`, or `adjusted`); explicitly unadjusted rows are rejected because split artifacts would poison the strategy dataset. Add `--research-ready` to fail when either ticker starts after the target start date, has too little history, or falls below the paired-overlap threshold. Defaults: `--target-start 2011-01-01 --min-years 10 --min-paired-overlap-pct 85`. Alpaca SIP minute history currently starts at `2016-01-04` for this dataset, so use `--target-start 2016-01-04` for the Alpaca-minute backfill and keep 2011 as an unresolved external data-source gap. Add `--json-output reports/csv-validation.json` when you want a machine-readable importability and research-readiness verdict.

`import:csv` imports a local adjusted OHLCV CSV into the running API by file path, then writes a fresh coverage report. CSVs without an `adjusted` column are treated as adjusted by contract; CSVs with `adjusted=0` or other falsey values fail validation. Use `--research-ready` when importing the real backfill so short/incomplete files fail before they touch the database. Research-ready validation also checks SOXL/SOXS paired timestamp overlap; the default minimum is 85% because the current Alpaca SIP backfill has sparse unmatched minutes, and you can tune it with `--min-paired-overlap-pct`. Use `--replace-bars` when importing a full historical dataset so old sample/cache bars do not remain mixed into the chart cache. Replacement is blocked while active labels exist unless you also pass `--force-replace-bars`; export a backup first, then run `labels:integrity` afterward because labels/trades are not deleted.

`labels:integrity` checks existing labels against the current bar cache and writes a report for missing candles, stale bar indexes, chart-price mismatches, feature snapshot issues, training eligibility mismatches, state-machine conflicts, and trade-ledger consistency. Run it after replacing bars. `labels:repair` updates labels that still point to an existing candle but have stale bar indexes or chart prices after a data replacement; it does not repair labels whose candles are missing.
